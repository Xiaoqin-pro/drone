...
for i=1:height(manifest)
    ...
    for routeIndex=1:numel(candidateRoutes)
        ...
        for sourceIndex=1:n
            ...
            localTravel=instance.costMatrix(prev,route(k))+instance.costMatrix(route(k),next);
            sourceRows{end+1,1}={manifest.name(i),routeIndex,string(candidateLabels{routeIndex}), ...
                ...
                max(0,removalSaving(k)),slack,max(0,rec(4)),max(0,rec(5)),localTravel, ...
...
sourceSummary=cell2table(vertcat(sourceRows{:}),'VariableNames',{ ...
    'instance','route_index','route_source','seed','source_node','source_cost','source_gain','removal_saving','slack','late','waiting', ...
    'local_travel','is_oracle_source','actionable'});
