function [BestSol,BestCost,stats,history] = AffectedRegionPSO_V2(model,cache, ...
    incumbentRoute,freeIDs,MaxIt,nPop,w,wdamp,c1,c2,maxFE)
%AFFECTEDREGIONPSO_V2 统一partial random-key编码的受影响区域PSO
%   固定客户使用incumbent route anchor key；free客户使用PSO变量。
...
fixedIDs=activeIDs(~ismember(activeIDs,freeIDs));
nVar=numel(freeIDs); VarMin=zeros(1,nVar); VarMax=ones(1,nVar);
VelMax=0.20*ones(1,nVar); VelMin=-VelMax;
fixedKeys=FixedAnchors(incumbentRoute,fixedIDs);
incumbentPosition=FreeKeys(incumbentRoute,freeIDs);
...
% 先把直接Repair incumbent作为已知候选，不允许搜索变差。
...
for i=1:nPop
    ...
    route=DecodePartial(pos,activeIDs,fixedIDs,fixedKeys,freeIDs);
    ...
    if particles(i).Cost<GlobalBest.Cost, GlobalBest=particles(i).Best; end
end
...
for it=1:MaxIt
    ...
    for i=1:nPop
        ...
        route=DecodePartial(particles(i).Position,activeIDs,fixedIDs,fixedKeys,freeIDs);
        [particles(i).Cost,particles(i).Detail]=EvaluateSchedule(route,model,cache); functionEvaluations=functionEvaluations+1;
        if particles(i).Detail.isFeasible && isinf(firstFeasible), firstFeasible=functionEvaluations; end
...
BestSol=GlobalBest; BestSol.Route=BestSol.Detail.routeIDs; stats.functionEvaluations=functionEvaluations; stats.firstFeasibleEvaluation=firstFeasible; stats.freeIDs=freeIDs; stats.fixedIDs=fixedIDs;
end

function keys=FixedAnchors(route,ids)
keys=zeros(1,numel(ids)); n=numel(route);
for k=1:numel(ids)
    pos=find(route==ids(k),1); if isempty(pos), keys(k)=rand; else, keys(k)=pos/(n+1); end
end
end
